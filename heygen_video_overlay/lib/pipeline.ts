...
export async function generate(
  ...
  // 1. HeyGen avatar (paid) — opaque webm carrying the speech audio.
  onStep?.("avatar");
  ...
  // 3. Transcribe the ORIGINAL audio (free) — word timestamps for captions.
  onStep?.("transcribe");
  await run(["transcribe", avatarWebm, "--model", "base"], work);
  const transcript = JSON.parse(await readFile(path.join(work, "transcript.json"), "utf8"));
  const speechMs = durationMs(transcript);

  // 4. Stage assets the composition reads at fixed paths. HyperFrames variables are scalars only
  //    (string/number/color/boolean/enum), so media + the caption word-array go through files in
...
// transcript.json word timestamps are seconds; total speech length = last word end.
function durationMs(transcript: { words?: { end?: number }[] }): number {
